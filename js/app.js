...
let jurisdictions=[],position=null,boundaries=[],plateImages={},locating=false,activeTab='United States';
let state=loadState();
const $=id=>document.getElementById(id);

function defaults(){return{activeTrip:null,trips:[],currentJurisdictionId:null};}
function loadState(){try{return{...defaults(),...(JSON.parse(localStorage.getItem(STORAGE_KEY))||JSON.parse(localStorage.getItem('plateQuest.v8'))||JSON.parse(localStorage.getItem('plateQuest.v7'))||{})};}catch{return defaults();}}
...
function plateVisual(jurisdiction,className='plate-image'){const src=plateImages[jurisdiction.id];return src?`<img class="${className}" src="./${esc(src)}" alt="${esc(jurisdiction.name)} license plate" loading="lazy">`:`<span class="mini-plate">${esc(jurisdiction.abbr)}</span>`;}
...
function renderGroup(seen){
 ...
 $('countryGroups').innerHTML=items.length?`<div class="jurisdiction-grid">${items.map(j=>`<button class="jurisdiction ${seen.has(j.id)?'collected':''}" data-id="${j.id}" ${seen.has(j.id)?'disabled':''}>${plateVisual(j)}<strong>${esc(j.name)}</strong></button>`).join('')}</div>`:'<div class="empty-state">No matching plates.</div>';
}
function renderCollected(currentTrip){if(!currentTrip?.sightings.length){$('collectedList').className='empty-state';$('collectedList').textContent='Tap a plate or map region to collect it.';return;}$('collectedList').className='';$('collectedList').innerHTML=currentTrip.sightings.slice().reverse().map(s=>{const j=jurisdictions.find(item=>item.id===s.jurisdictionId)||{id:s.jurisdictionId,name:s.name,abbr:''};return`<div class="collected-item">${plateVisual(j,'collected-plate')}<span class="collected-copy"><strong>${esc(s.name)}</strong><small>${Math.round(s.distance)} miles</small></span><strong>${s.points} pts</strong></div>`;}).join('');}
